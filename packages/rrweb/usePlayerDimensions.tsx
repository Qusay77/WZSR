...
const usePlayerDimensions = (PlayerRef: HTMLDivElement | null) => {
	...
	useEffect(() => {
		...
		PlayerInstance.addEventListener("resize", (replayDimensions) => {
			updatePlayerDimensions(replayDimensions, rr_player, player__frame);
		});
		const windowResizeCallBack = () => windowResize(rr_player, player__frame);
		window.addEventListener("resize", windowResizeCallBack);
		return () => window.removeEventListener("resize", windowResizeCallBack);
	}, [PlayerInstance]);

	const windowResize = (Player: HTMLElement, Frame: HTMLElement) => {
